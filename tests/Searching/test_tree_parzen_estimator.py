...
class TestTreeParzenEstimator(unittest.TestCase):
    def setUp(self):
        self.param_space = {
            "param1": [1, 2, 3, 4, 5],
            "param2": ["a", "b", "c"],
        ...
        self.test_filepath = "test_tpe_state.pkl"

    def tearDown(self):
        if os.path.exists(self.test_filepath):
            os.remove(self.test_filepath)
        # Reset random seed if necessary for other tests, though each test method should be isolated.
        random.seed(None)

    def test_initialization_valid(self):
        self.assertEqual(self.tpe.n_initial_points, 3)
        self.assertEqual(self.tpe.max_trials, 10)
        self.assertEqual(self.tpe.gamma_ratio, 0.25)
        self.assertEqual(self.tpe.n_ei_candidates, 5)
        self.assertEqual(self.tpe.param_space, self.param_space)
        self.assertEqual(self.tpe.trial_count, 0)
        self.assertEqual(self.tpe.best_score, -float("inf"))
        self.assertIsNotNone(
            self.tpe.best_params
        )  # Should be initialized to a random set
        self.assertTrue(all(p in self.param_space for p in self.tpe.best_params.keys()))
    ...
    def test_initialization_low_n_initial_points(self):
        ...
        self.assertEqual(tpe_low_init.n_initial_points, 2)
        tpe_low_init_0 = TreeParzenEstimator(n_initial_points=0, verbose=True)
        self.assertEqual(tpe_low_init_0.n_initial_points, 2)
    ...
    def test_generate_random_params(self):
        ...
        for p_name, p_value in params.items():
            self.assertIn(p_name, self.param_space)
            self.assertIn(p_value, self.param_space[p_name])
    ...
    def test_reset(self):
        ...
        self.tpe.reset()
        self.assertEqual(self.tpe.trial_count, 0)
        self.assertEqual(self.tpe.history, [])
        self.assertEqual(self.tpe.scores, [])
        self.assertEqual(self.tpe.best_score, -float("inf"))
        # best_params should be re-initialized to a random configuration if param_space was set
        self.assertIsNotNone(self.tpe.best_params)
        self.assertEqual(
        ...
        tpe_no_space.reset()
        self.assertIsNone(tpe_no_space.best_params)
    ...
    def test_get_next_params_tpe_phase_not_enough_trials_fallback(self):
        ...
        self.assertEqual(tpe_few.trial_count, 1)
        ...
        self.assertIsInstance(params_next, dict)
    ...
    def test_update(self):
        ...
        self.assertEqual(self.tpe.trial_count, 1)
        self.assertEqual(len(self.tpe.history), 1)
        self.assertEqual(self.tpe.history[0], params)
        self.assertEqual(len(self.tpe.scores), 1)
        self.assertEqual(self.tpe.scores[0], score)
        self.assertEqual(self.tpe.best_score, score)
        self.assertEqual(self.tpe.best_params, params)

        # Update with worse score
        params_worse = {"param1": 2, "param2": "b", "param3": 0.5}
        score_worse = 5.0
        self.tpe.update(params_worse, score_worse)
        self.assertEqual(self.tpe.trial_count, 2)
        self.assertEqual(self.tpe.best_score, score)  # Best should not change
        self.assertEqual(self.tpe.best_params, params)
        ...
        self.assertEqual(self.tpe.trial_count, 3)
        self.assertEqual(self.tpe.best_score, score_better)
        self.assertEqual(self.tpe.best_params, params_better)
    ...
    def test_update_first_call_sets_best_params(self):
        ...
        self.assertIsNotNone(tpe_first.best_params)  # Initialized randomly
        initial_random_best_score = tpe_first.best_score  # Should be -inf
        self.assertEqual(initial_random_best_score, -float("inf"))

        p = tpe_first.get_next_params()
        s = 5.0
        tpe_first.update(p, s)
        self.assertEqual(tpe_first.best_params, p)
        self.assertEqual(tpe_first.best_score, s)
    ...
    def test_save_and_load_state(self):
        ...
        self.tpe.save_state(self.test_filepath)
        ...
        new_tpe.load_state(self.test_filepath)

        self.assertEqual(new_tpe.param_space, self.tpe.param_space)
        self.assertEqual(new_tpe.history, self.tpe.history)
        self.assertEqual(new_tpe.scores, self.tpe.scores)
        self.assertEqual(new_tpe.best_params, self.tpe.best_params)
        self.assertEqual(new_tpe.best_score, self.tpe.best_score)
        self.assertEqual(new_tpe.trial_count, self.tpe.trial_count)
        self.assertEqual(new_tpe.n_initial_points, self.tpe.n_initial_points)
        self.assertEqual(new_tpe.max_trials, self.tpe.max_trials)
        self.assertEqual(new_tpe.gamma_ratio, self.tpe.gamma_ratio)
        self.assertEqual(new_tpe.n_ei_candidates, self.tpe.n_ei_candidates)
        self.assertEqual(new_tpe.verbose, self.tpe.verbose)
        self.assertEqual(
            random.getstate(), original_random_state
        )  # Check if random state was restored
    ...
    def test_load_state_file_not_found(self):
        ...
        self.assertEqual(tpe_load.trial_count, 0)
    ...
    def test_get_best_params_and_score_no_trials(self):
        ...
        self.assertIsNotNone(tpe_no_trials.get_best_params())
        self.assertTrue(
            all(p in self.param_space for p in tpe_no_trials.get_best_params().keys())
        )
        self.assertEqual(tpe_no_trials.get_best_score(), -float("inf"))

    # def test_get_best_params_uninitialized(self):
    #     tpe_uninit = TreeParzenEstimator()
    #     with self.assertRaisesRegex(
    #         RuntimeError, "Optimizer not initialized"
    #     ):  # Check for specific error message if desired
    #         tpe_uninit.get_best_params()

    def test_is_finished(self):
        self.assertFalse(self.tpe.is_finished())
        self.tpe.trial_count = self.tpe.max_trials
        self.assertTrue(self.tpe.is_finished())
    ...
    def test_tpe_logic_runs(self):
        ...
        self.assertGreaterEqual(self.tpe.trial_count, self.tpe.n_initial_points)

        # Next call should use TPE
        try:
            tpe_params = self.tpe.get_next_params()
            self.assertIsInstance(tpe_params, dict)
            for p_name, p_value in tpe_params.items():
                self.assertIn(p_name, self.param_space)
                self.assertIn(p_value, self.param_space[p_name])
        except Exception as e:
            self.fail(f"TPE get_next_params failed after initial points: {e}")
        ...
        while not self.tpe.is_finished():
            try:
                ...
                self.fail(f"get_next_params or update failed during budgeted run: {e}")
        self.assertEqual(self.tpe.trial_count, self.tpe.max_trials)
    ...
    def test_calculate_param_prob_laplace_smoothing(self):
        ...
        # (0+1)/(0+1*3) = 1/3
        self.assertAlmostEqual(prob_x_empty_obs, (0 + 1) / (0 + 3 * 1))
    ...
    def test_sample_param_value(self):
        ...
        self.assertTrue(all(s in observed for s in samples_from_observed))
        self.assertIn(10, samples_from_observed)
        self.assertIn(20, samples_from_observed)
        self.assertNotIn(30, samples_from_observed)  # 30 is not in observed

        # Sample from all_possible if observed is empty
        samples_from_all = [
            self.tpe._sample_param_value([], all_possible) for _ in range(30)
        ]
        self.assertTrue(all(s in all_possible for s in samples_from_all))
        # Check if all values appear with some frequency (probabilistic)
        self.assertTrue(any(s == 10 for s in samples_from_all))
        self.assertTrue(any(s == 20 for s in samples_from_all))
        self.assertTrue(any(s == 30 for s in samples_from_all))
    ...
    def test_gamma_split_logic(self):
        ...
        self.assertEqual(num_total_trials, 10)
        ...
        try:
            ...
            self.fail(f"get_next_params failed in gamma_split_logic test: {e}")
        ...
        try:
            ...
            self.assertIsNotNone(next_p_low)
        except Exception as e:
            self.fail(
        ...
        try:
            ...
            self.assertIsNotNone(next_p_high)
        except Exception as e:
            self.fail(
